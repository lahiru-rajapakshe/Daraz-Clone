...
public class CategoryServices extends BaseServlet {
    ...
    public void deleteCategory() throws ServletException, IOException {
        ...
        if (numberOfBooks > 0) {
            message = "Could not delete the category id (ID: %d) because it currently contains some books";
            message= String.format(message, numberOfBooks);
        } else {
            categoryDAO.delete(categoryId);
            message = "the category with id " + categoryId + " has been removed succesfully !";

        }
        listCategory(message);


         message = "The category id " + categoryId + "has been deleted !";
        listCategory(message);
